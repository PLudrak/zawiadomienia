...
def convert_adress(SWDE_adres: str, kody_pocztowe: dict):
    ...
    try:
        ...
        if (
            len(kod_miejscowosc) < 2 and "-" not in kod_miejscowosc[0]
        ):  # Sprawdza czy kod pocztowy nie znajduje się w zmiennej
        ...
        while True:
            ...
            print(
                "ULICA NUMER m.LOKAL; KOD MIEJSCOWOŚĆ lub \n[3] Zatwierdź brak adresu\n[4] Zagranica"
            )
            ...
            if adres_input.upper() == "3":
                ...
                try:
                    ulica, nr_a, nr_l, kod, miejscowosc = convert_adress(
                        adres_input, kody_pocztowe
                    )
                    return ulica, nr_a, nr_l, kod, miejscowosc
                except:
                    print("Nierozpoznano adresu lub wyobru")
...
def missing_postcode(miejscowosc: str, adres, kody_pocztowe):
    ...
    while True:
        ...
        if ostatni_kod:
            print(
                f"[1] Użyj ostaniego kodu dla {miejscowosc}: {kody_pocztowe[miejscowosc]}"
            )
...
def search_postcode(miejscowosc: str):
    ...
    if postcode:
        print(
            f"Kod pocztowy dla adresu {miejscowosc} to {postcode}:\n [1] zatwierdź lub wpisz poprawny kod"
        )
...
def has_empty_critical(nowy_rekord, df_row, critical_keys):
    ...
    return any(
        is_empty_val(nowy_rekord[k]) or is_empty_val(df_row[k]) for k in critical_keys
    )
...
def find_similar_record(
    ...
    for _, row in df.iterrows():
        podobienstwa = {
            key: compare_values(row[key], nowy_rekord[key], prog) for key in keys
        }
        ...
        if (
            first_similar is None
            and crit_match
            and not has_empty_critical(nowy_rekord, row, critical_keys)
        ):
...
def handle_similar_records(old: pd.Series, new: dict, identical=False, auto=True):
    ...
    while True:
        choice = c_input()
        if choice == "1":
            new["ID_ZASTAPIENIA"] = old_dict["ID_ORYGINALNE"]
            return new
        if choice == "2":
            return new
        if choice == "3":
